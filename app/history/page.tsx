      ...
      {(!rows || rows.length === 0) ? (
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {rows.map(r => {
            ...
            const items = (payload?.items ?? []) as {title:string;duration:number}[];
            const total = items.reduce((s,i)=>s+(i.duration??0),0);
            const start = payload?.startTime ?? "09:00";
            const end = (() => {
              const [h,m] = start.split(":").map((n:number)=>parseInt(String(n),10));
              return minutesToHHMM(h*60+m+total);
            })();
            const t = Buffer.from(JSON.stringify(payload)).toString("base64")
            ...
            return (
              <li key={r.id} className="rounded-lg border bg-white p-4 shadow-sm">
                ...
                <div className="mt-1 text-sm text-gray-600">開始 {start} / 終了 {end} / 合計 {total}分</div>
